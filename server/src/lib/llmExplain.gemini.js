// server/src/lib/llmExplain.gemini.js
...
async function geminiGenerate(prompt) {
  ...
  const apiKey =
    String(llm.geminiApiKey || llm['gemini-api-key'] || process.env.GEMINI_API_KEY || '').trim();
  if (!apiKey) return null;

  const base = String(llm.geminiBaseUrl || 'https://generativelanguage.googleapis.com').replace(/\/+$/, '');
  const model = String(llm.geminiModel || 'gemini-2.5-flash').trim();
    ...
    const r = await fetchFn(url, {
      ...
      body: JSON.stringify({
        ...
        generationConfig: {
          ...
          maxOutputTokens: Number(llm.maxTokens || 2540)
        }
      })
    });
...
function buildPrompt({ index, line, message, snippet }) {
  ...
  return (
    ...
    `"${errorText}"\n\n` +
    `If there are missing a required parameter or Unknown identifier, try to found the solution (What missing?). \n` +
    `You can find (What have oped and it not closed) in the snippet the context of the error:\n` +
    `"\n${snippet}\n"\n\n` +
